...
def delete_record(model_class, window_title, label_text):
    def submit_deletion():
        ...
        if record:
            ...
            session.commit()
            messagebox.showinfo("Brawo!", f"Pracownik o ID {record_id} został usunięty pomyślnie.")
        else:
            messagebox.showerror("Błąd!", f"Brak pracownika o ID {record_id}.")
...
def add_employee():
    ...
    add_employee_window.title("Dodaj nowego pracownika")
    ...
    tk.Label(add_employee_window, text="Szerokość geograficzna:").grid(row=5, column=0)
    latitude_entry = tk.Entry(add_employee_window)
    latitude_entry.grid(row=5, column=1)

    tk.Label(add_employee_window, text="Długość geograficzna:").grid(row=6, column=0)
    longitude_entry = tk.Entry(add_employee_window)
    longitude_entry.grid(row=6, column=1)

    submit_button = tk.Button(add_employee_window, text="Dodaj", command=submit_employee)
...
def edit_employee():
        ...
        def update_employee():
            try:
                ...
                if employee:
                    ...
                    messagebox.showerror("Błąd!", "Nie znaleziono pracownika o podanym ID.")
            except ValueError:
                messagebox.showerror("Błąd!", "Wprowadź odpowiednie ID.")

        id_window = Toplevel()
        id_window.title("Wprowadź ID pracownika")

        tk.Label(id_window, text="ID pracownika:").pack()
        id_entry = tk.Entry(id_window)
        id_entry.pack()

        submit_button = tk.Button(id_window, text="Edytuj", command=update_employee)
        submit_button.pack()
    ...
    def show_edit_form(employee):
        def save_changes():
            ...
            session.commit()
            edit_window.destroy()
            messagebox.showinfo("Brawo!", "Dane pracownika zostały edytowane poprawnie.")

        edit_window = Toplevel()
        edit_window.title("Edytuj dane pracownika")
...
def remove_employee():
    delete_record(Pracownik, "Usuń pracownika", "Wprowadź ID użytkownika:")
